...
- Follow up with \`sdamgia_get_problem\` to see full problem details`,
        ...
        try {
            ...
            if (searchResults.length === 0) {
                const output = formatResponse({ problem_ids: [], total: 0, message: "No problems found" }, response_format, () => formatSearchResultsMarkdown(condition_text, [], subject));
                return {
                    content: [{ type: "text", text: output }],
            ...
            const problems = await Promise.all(searchResults.slice(0, Math.min(30, limit * 3)).map(async (id) => {
                try {
            ...
            }, response_format, () => formatSearchResultsMarkdown(condition_text, matches, subject));
            return {
                content: [{ type: "text", text: output }],
                structuredContent: {
                    matches: matches.map(m => ({ problem_id: m.id, similarity: m.score })),
