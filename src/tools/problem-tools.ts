...
import {
  ...
  SearchByTextInputSchema,
  BatchGetProblemsInputSchema
} from "../schemas/input-schemas.js";
import {
  formatProblemMarkdown,
  formatSearchResultsMarkdown,
  formatBatchProblemsMarkdown,
  formatResponse
 ...
 */
export function registerProblemTools(server: McpServer, client: SdamGiaClient) {
...
- Subject-specific terminology works best for quality results`,
    ...
    async ({ subject, query, limit, response_format }) => {
      try {
        ...
        const output = formatResponse(
          { problem_ids: problemIds, total: problemIds.length },
          response_format,
          () => formatSearchResultsMarkdown(query, results, subject)
        );
...
- All problems in batch are fetched in parallel for speed`,
    ...
    async ({ subject, problem_ids, response_format }) => {
      try {
        const problems = await Promise.all(
          problem_ids.map(id => client.getProblemById(subject, id))
        );

        const output = formatResponse(
          { problems, total: problems.length },
          response_format,
          () => formatBatchProblemsMarkdown(problems)
        );
