...
import {
  ...
  GetCategoryProblemsInputSchema,
  GetTestInputSchema
} from "../schemas/input-schemas.js";
import {
  formatCatalogMarkdown,
  formatProblemIdsMarkdown,
  formatResponse
 ...
 */
export function registerCatalogTools(server: McpServer, client: SdamGiaClient) {
    ...
    async ({ subject, response_format }) => {
      try {
        ...
        const output = formatResponse(
          { catalog, total_topics: catalog.length },
          response_format,
...
- "Build a randomized problem set from category 67890"`,
    ...
    async ({ subject, category_id, limit, response_format }) => {
      try {
        ...
        const output = formatResponse(
          { problem_ids: problemIds, category_id, total: problemIds.length },
          response_format,
          () => formatProblemIdsMarkdown(problemIds, `Category ${category_id} Problems`)
        );
...
- Reviewing complete test content before exam day`,
    ...
    async ({ subject, test_id, response_format }) => {
      try {
        ...
        const output = formatResponse(
          { problem_ids: problemIds, test_id, total: problemIds.length },
          response_format,
          () => formatProblemIdsMarkdown(problemIds, `Test ${test_id} Problems`)
        );
